 ...
 * one source base for multiple platforms. 
 * 
 * Well I thought I may have been able to use some of the code and work from the Game using the CandiTV Platform version, but this wasn’t so simple due to 
 * dependent classes, framework and the timeline. Therefore I had to rewrite a new version built using pure AS3 in Flash Builder.
 * 
 * Code & Algoritms
 * 
...
/**
 * Once the object is constructed the system just has to shuffle the image and this goes through a few processes. It needs to work out the size of each tile 
 * depending on the rowCount and columnCount passed into the function. Then it needs to slice the bitmap using these values. 
 * 
 * The algorithm creates a tile as it iterates through the column and row counts. It uses the bitmapdata stored in the Bitmap object to gather the pixels 
 * within the current sector of the tile and stores the tile data into a sprite. Each of the tiles are then stored into a global array.
*/
 ...
 * Improvements
 * 
 * There is always something to improve when developing algorithms and processes, speed and performances is always a good feature to improve, as the algorithm to
 * randomly sort an array can be written in many ways and each way can process at different rates. 
 * 
 * One of the major improvements is to remove the need of the Caurina library. As I stated, I wanted this to be a class and component to be added to a DisplayList
 * in Flash, without the dependencies of Flex components, Flash Timeline, QNX framework etc, but this class currently depends on the Caurina library, so if I was 
 * to port this class to another project I will need to add the Caurina library to the project too.
 * 
 * To do this, I will probably have to write my own tweening and animation algorithms to move the tiles in the component. Maybe eventually write my own tween engine
 * that can be used across different applications.
 * 
 * Overall this class can now be treated as a component and can easily be added to a Flash Application; either it’s a mobile application for the BlackBerry Playbook,
 * iPad, iPhone or any Android device, but also desktop AIR applications and applications in the web browser.					
 */ 
